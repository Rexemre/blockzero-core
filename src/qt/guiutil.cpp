...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BLOZ address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no payment URI (bloz: or legacy bitcoin:)
    if (!uri.isValid() || (uri.scheme() != QString("bloz") && uri.scheme() != QString("bitcoin")))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bloz:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
