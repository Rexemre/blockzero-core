...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-devfundpercent=<n>", strprintf("Percentage of the block subsidy contributed to the Block Zero Development & Growth Fund when mining, once the fund is active. Values below the consensus minimum are raised to it. (default: %d)", node::DEFAULT_DEV_FUND_PERCENT), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    {
        const auto dev_fund_percent = args.GetIntArg("-devfundpercent", node::DEFAULT_DEV_FUND_PERCENT);
        if (dev_fund_percent < 0 || dev_fund_percent > 100) {
            return InitError(strprintf(_("Specified -devfundpercent (%d) must be between 0 and 100"), dev_fund_percent));
        }
    }
